...
num_x=floor(raw_pic_size(1)/40);
num_y=floor(raw_pic_size(2)/40);
...
for id_x=1:num_x
   for id_y=1:num_y
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       %取出每一次检测的单元
       picture=picture_raw(id_x*40-39:id_x*40,id_y*40-39:id_y*40,:);
       ...
       %提取颜色向量，结果存在f里面
        ...
        %判定，如果是人脸单元的话，就在图中标出
        if(d<=0.4500)
            painting=Circle(painting,  id_x*40-39,  id_y*40-39,  id_x*40,  id_y*40);
        end
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   end
end
